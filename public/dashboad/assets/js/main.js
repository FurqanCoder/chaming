    ...
    $(document).ready(function () {
        ...
        if ($('.editor').length) {
            ...
            document.querySelectorAll('.editor').forEach((node, index) => {
                const editorName = node.dataset.name; // Get the value of data-name attribute
        
                ClassicEditor
                    .create(node, {})
                    .then(newEditor => {
                        window.editors[editorName] = newEditor; // Use the name as the key in the editors object
                    });
            });}
